...
const BOONS = [
  ...
  {name:'Max Damage',tag:'OFFENSE',icon:'⬆',desc:'Raise only the top end of damage. Max 4.',apply(upg){upg.damageCeilTier=Math.min(4,(upg.damageCeilTier||0)+1);}},
  {name:'Damage Floor',tag:'OFFENSE',icon:'⬇',desc:'Raise the low end of damage at 2/3 strength. Max 4.',apply(upg){upg.damageFloorTier=Math.min(4,(upg.damageFloorTier||0)+1);}},
  ...
  {name:'Critical Hit',tag:'OFFENSE',icon:'💥',desc:'+25% crit chance. Max 3.',apply(upg){upg.critTier=Math.min(3,upg.critTier+1);upg.critChance=Math.min(0.75,0.25*upg.critTier);upg.critDamageBonus=Math.max(CRIT_DAMAGE_BASE_BONUS, upg.critDamageBonus ?? CRIT_DAMAGE_BASE_BONUS);}},
