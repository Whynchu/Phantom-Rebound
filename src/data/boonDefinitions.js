...
const BOONS = [
  {name:'Rapid Fire', tag:'OFFENSE', icon:'⚡', damageRole:'neutral', desc:'Next fire-rate tier.', apply(upg){ if(upg.spsTier<SPS_LADDER.length-1){upg.spsTier++;upg.sps=SPS_LADDER[upg.spsTier];}}},
  ...
  {name:'Critical Damage',tag:'OFFENSE',icon:'✦',damageRole:'ceiling',desc:'Crits hit harder with diminishing returns. Max 4.',apply(upg){upg.critDamageTier=Math.min(CRIT_DAMAGE_TIER_BONUS.length, (upg.critDamageTier||0)+1);upg.critDamageBonus=getCritDamageBonusFromTier(upg.critDamageTier);raiseDamageFloor(upg);}},
  ...
  {name:'Extra Life',tag:'SURVIVE',icon:'◉',desc:'+max HP and heal on pickup. Slightly slows movement.',apply(upg, state){upg.extraLifeTier++;const idx=Math.min(EXTRA_LIFE_GAINS.length-1, upg.extraLifeTier-1);const heal=EXTRA_LIFE_GAINS[idx];state.maxHp+=heal;state.hp=Math.min(state.hp+heal,state.maxHp);upg.extraLifeSlowMult=(upg.extraLifeSlowMult||1)*EXTRA_LIFE_SLOW_PER_TIER;}},
  ...
  {name:'Late Bloom',tag:'OFFENSE',icon:'🌱',damageRole:'ceiling',desc:'Scales damage by room. -6% speed.',apply(upg){if(hasLateBloomVariant(upg))return; upg.lateBloomVariant='power'; raiseDamageFloor(upg);}},
  {name:'Swift Bloom',tag:'UTILITY',icon:'🍃',damageRole:'neutral',desc:'Scales speed by room. +6% damage taken.',apply(upg){if(hasLateBloomVariant(upg))return; upg.lateBloomVariant='speed';}},
  {name:'Guard Bloom',tag:'SURVIVE',icon:'🛡️',damageRole:'defense',desc:'Scales defense by room. -6% damage.',apply(upg){if(hasLateBloomVariant(upg))return; upg.lateBloomVariant='defense';}},
  ...
  {name:'Spread Shot',tag:'OFFENSE',icon:'⬄',damageRole:'neutral',desc:'3-shot cone. +1 charge cost, +35% spread pellet damage, +1 spread pierce.',apply(upg){if(upg.spreadShot)return; upg.spreadShot=true; upg.spreadShotDamageMult=1.35; upg.spreadShotPierceBonus=1; syncChargeCapacity(upg);}},
...
const LEGENDARY_SEQUENCES = [
  ...
  },
  {
    id: 'conduit',
    check: () => false,
    boon: { name:'CONDUIT', tag:'LEGENDARY', icon:'⚡', desc:'Lightning arcs between your orbs, shocking anything caught in the web. Stacks with all orb boons. Exclusive.', requires: upg => upg.orbitSphereTier >= 2 && !upg.berserker && upg.titanTier === 0, apply(upg){ if(upg.conduit || upg.berserker || upg.titanTier > 0 || upg.orbitSphereTier < 2) return; upg.conduit = true; upg.conduitArcDmg = 6; upg.conduitArcTickMs = 120; } },
  },
  {
    id: 'ghostFlow',
