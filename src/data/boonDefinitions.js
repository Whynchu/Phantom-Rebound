...
const BOONS = [
  ...
  {name:'Snipe Shot',tag:'OFFENSE',icon:'🎯',damageRole:'ceiling',desc:'+shot size, speed, and damage. Leans high end.',apply(upg){upg.snipePower=Math.min(3,upg.snipePower+1);}},
  {name:'Max Damage',tag:'OFFENSE',icon:'⬆',damageRole:'ceiling',desc:'Raise only the top end of damage. Max 4.',apply(upg){upg.damageCeilTier=Math.min(4,(upg.damageCeilTier||0)+1);}},
  ...
  {name:'Heavy Rounds',tag:'OFFENSE',icon:'🔨',damageRole:'ceiling',desc:'-40% fire rate, +65% damage. Max 3.',apply(upg){if(upg.heavyRoundsTier>=3)return; upg.heavyRoundsTier++; upg.heavyRoundsFireMult*=0.60; upg.heavyRoundsDamageMult*=1.65;}},
  {name:'Dense Core',tag:'OFFENSE',icon:'◈',damageRole:'ceiling',desc:'Less max charge, more damage. Max 4.',apply(upg){if(upg.denseTier>=4)return; upg.denseTier++; upg.denseDamageMult=DENSE_CORE_DAMAGE_MULTS[Math.min(DENSE_CORE_DAMAGE_MULTS.length - 1, upg.denseTier - 1)]; syncChargeCapacity(upg);}},
  ...
  {name:'Late Bloom',tag:'OFFENSE',icon:'🌱',damageRole:'ceiling',desc:'Scales damage by room. -6% speed.',apply(upg){if(hasLateBloomVariant(upg))return; upg.lateBloomVariant='power';}},
  ...
  {name:'Escalation',tag:'OFFENSE',icon:'📈',damageRole:'ceiling',desc:'+3% damage per kill this room. Max +60%.',apply(upg){if(upg.escalation)return; upg.escalation=true;}},
  ...
  {name:'Glass Cannon',tag:'OFFENSE',icon:'⚗',damageRole:'ceiling',desc:'Big damage, shrinking HP pool. Max 5.',apply(upg,state){const damageMults=[1.12,1.18,1.30,1.45,1.65];if(upg.glassCannonTier>=damageMults.length)return; const tier=upg.glassCannonTier; upg.glassCannonTier++; upg.playerDamageMult*=damageMults[tier]; state.maxHp=Math.max(20,Math.round(state.maxHp*0.92)); state.hp=Math.min(state.hp,state.maxHp);}},
