...
function raiseDamageFloor(upg, tiers = 1) {
  upg.damageFloorTier = Math.min(4, (upg.damageFloorTier || 0) + tiers);
}

const BOONS = [
  {name:'Rapid Fire', tag:'OFFENSE', icon:'⚡', damageRole:'neutral', desc:'Advance fire rate one tier.', apply(upg){ if(upg.spsTier<SPS_LADDER.length-1){upg.spsTier++;upg.sps=SPS_LADDER[upg.spsTier];}}},
  ...
  {name:'Backshot',tag:'OFFENSE',icon:'↕',damageRole:'neutral',desc:'Add one rear shot.',apply(upg){upg.dualShot=1;syncChargeCapacity(upg);}},
  {name:'Snipe Shot',tag:'OFFENSE',icon:'🎯',damageRole:'ceiling',desc:'Increase shot size, speed, and damage.',apply(upg){upg.snipePower=Math.min(3,upg.snipePower+1);raiseDamageFloor(upg);}},
  {name:'Max Damage',tag:'OFFENSE',icon:'⬆',damageRole:'ceiling',desc:'Raise damage ceiling only, max 4.',apply(upg){upg.damageCeilTier=Math.min(4,(upg.damageCeilTier||0)+2);}},
  {name:'Damage Floor',tag:'OFFENSE',icon:'⬇',damageRole:'floor',desc:'Raise damage floor only, max 4.',apply(upg){upg.damageFloorTier=Math.min(4,(upg.damageFloorTier||0)+1);}},
  ...
  {name:'Bigger Bullets',tag:'OFFENSE',icon:'🔵',damageRole:'neutral',desc:'Increase shot size with diminishing returns.',apply(upg){upg.biggerBulletsTier++;const scale=getHyperbolicScale(upg.biggerBulletsTier);upg.shotSize=scale*(upg.biggerBulletsTier===1?1.06:1);}},
  {name:'Faster Bullets',tag:'OFFENSE',icon:'💨',damageRole:'neutral',desc:'Increase shot speed with diminishing returns.',apply(upg){upg.fasterBulletsTier++;const scale=getHyperbolicScale(upg.fasterBulletsTier);upg.shotSpd=scale*(upg.fasterBulletsTier===1?1.06:1);}},
  {name:'Critical Hit',tag:'OFFENSE',icon:'💥',damageRole:'ceiling',desc:'Increase crit chance with diminishing returns.',apply(upg){upg.critTier=Math.min(CRIT_CHANCE_TIER_BONUS.length, (upg.critTier||0)+1);upg.critChance=getCritChanceFromTier(upg.critTier, (upg.miniTier || 0) * MINI_CRIT_CHANCE_PER_TIER);upg.critDamageBonus=Math.max(CRIT_DAMAGE_BASE_BONUS, upg.critDamageBonus ?? CRIT_DAMAGE_BASE_BONUS);raiseDamageFloor(upg);}},
  {name:'Critical Damage',tag:'OFFENSE',icon:'✦',damageRole:'ceiling',desc:'Increase crit damage with diminishing returns.',apply(upg){upg.critDamageTier=Math.min(CRIT_DAMAGE_TIER_BONUS.length, (upg.critDamageTier||0)+1);upg.critDamageBonus=getCritDamageBonusFromTier(upg.critDamageTier);raiseDamageFloor(upg);}},
  ...
  {name:'Capacity Boost',tag:'UTILITY',icon:'◆',desc:'+16% max charge and at least +2 charge per pick.',apply(upg){if((upg.chargeCapMult||1) >= MAX_CHARGE_CAP_MULT)return; upg.chargeCapTier++;upg.chargeCapMult = Math.min(MAX_CHARGE_CAP_MULT, 1 + upg.chargeCapTier * CHARGE_CAP_PCT);syncChargeCapacity(upg);}},
  {name:'Deep Reserve',tag:'UTILITY',icon:'▣',desc:'+flat charge, starts at +16 and caps at +120.',apply(upg){if((upg.chargeCapFlatBonus||0) >= MAX_DEEP_RESERVE_BONUS)return; upg.chargeCapFlatTier++;upg.chargeCapFlatBonus = Math.min(MAX_DEEP_RESERVE_BONUS, (upg.chargeCapFlatBonus||0) + Math.max(4, Math.round(16 / (1 + (upg.chargeCapFlatTier-1) * 0.28))));syncChargeCapacity(upg);}},
  {name:'Wider Absorb',tag:'UTILITY',icon:'🧲',desc:'Increase absorb range, max +60.',apply(upg){upg.absorbRange=Math.min(60,upg.absorbRange+15);}},
  {name:'Long Reach',tag:'UTILITY',icon:'➶',desc:'Shots travel farther before expiring.',apply(upg){upg.shotLifeTier++;upg.shotLifeMult=getHyperbolicScale(upg.shotLifeTier);}},
  {name:'Kinetic Harvest',tag:'UTILITY',icon:'🌀',desc:'Move to gain charge; low charge fills faster.',apply(upg){upg.kineticTier++;const baseMoveRate=getHyperbolicScale(upg.kineticTier)*0.45;const spsSynergy=1+(upg.spsTier||0)*0.15;upg.moveChargeRate=baseMoveRate*spsSynergy;}},
  ...
  {name:'Extra Life',tag:'SURVIVE',icon:'◉',desc:'+max HP, heals on pickup, and slows movement.',apply(upg, state){upg.extraLifeTier++;const idx=Math.min(EXTRA_LIFE_GAINS.length-1, upg.extraLifeTier-1);const heal=EXTRA_LIFE_GAINS[idx];state.maxHp+=heal;state.hp=Math.min(state.hp+heal,state.maxHp);upg.extraLifeSlowMult=(upg.extraLifeSlowMult||1)*EXTRA_LIFE_SLOW_PER_TIER;}},
  {name:'Ghost Velocity',tag:'SURVIVE',icon:'👻',desc:'Increase move speed with diminishing returns.',apply(upg){upg.speedTier++;upg.speedMult=getHyperbolicScale(upg.speedTier);}},
  ...
  {name:'Armor Weave',tag:'SURVIVE',icon:'🧱',desc:'Reduce damage taken: -18%, -36%, then -50%.',apply(upg){const multipliers=[1,0.82,0.64,0.5];upg.armorTier=Math.min(3,upg.armorTier+1);upg.damageTakenMult=multipliers[upg.armorTier];},evolvesWith:['Titan Heart'],evolvedVersion:{name:'Living Fortress',icon:'🧱+',desc:'Armor scales with missing HP.',apply(upg){const multipliers=[1,0.82,0.64,0.5];upg.armorTier=Math.min(3,upg.armorTier+1);upg.damageTakenMult=multipliers[upg.armorTier];upg.livingFortress=true;}}},
  ...
  {name:'Barrier Pulse',tag:'SURVIVE',icon:'⬡',desc:'Shield break grants +2 charge and magnet.',requires:upg=>upg.shieldTier>0,apply(upg){if(upg.barrierPulse)return; upg.barrierPulse=true;}},
  ...
  {name:'Orbit Spheres',tag:'UTILITY',icon:'🔮',desc:'+1 orbiting sphere. Max 5.',apply(upg){upg.orbitSphereTier=Math.min(5,upg.orbitSphereTier+1);}},
  ...
  {name:'Charged Orbs',tag:'OFFENSE',icon:'⚡',desc:'Orbs spend charge to fire nearby enemies.',requires:upg=>upg.orbitSphereTier>0,apply(upg){if(upg.chargedOrbs)return; upg.chargedOrbs=true;}},
  ...
  {name:'Orb Overcharge',tag:'OFFENSE',icon:'⚡⬆',desc:'Charged orb shots scale harder from charge.',requires:upg=>upg.chargedOrbs,apply(upg){if(upg.orbOvercharge)return; upg.orbOvercharge=true;}},
  ...
  {name:'Aegis Battery',tag:'OFFENSE',icon:'🔋',desc:'Ready shields boost returns; full shields fire bolts.',requires:upg=>upg.shieldTier>0,apply(upg){if(upg.aegisBattery)return; upg.aegisBattery=true; upg.aegisBatteryTimer=0;}},
  {name:'Heavy Rounds',tag:'OFFENSE',icon:'🔨',damageRole:'ceiling',desc:'-40% fire rate, +65% damage. Max 3.',apply(upg){if(upg.heavyRoundsTier>=3)return; upg.heavyRoundsTier++; upg.heavyRoundsFireMult*=0.60; upg.heavyRoundsDamageMult*=1.65; raiseDamageFloor(upg);}},
  {name:'Dense Core',tag:'OFFENSE',icon:'◈',damageRole:'ceiling',desc:'Less max charge, more damage. Max 4.',apply(upg){if(upg.denseTier>=4)return; upg.denseTier++; upg.denseDamageMult=DENSE_CORE_DAMAGE_MULTS[Math.min(DENSE_CORE_DAMAGE_MULTS.length - 1, upg.denseTier - 1)]; syncChargeCapacity(upg); raiseDamageFloor(upg);}},
  {name:'Echo Fire',tag:'OFFENSE',icon:'↺',damageRole:'neutral',desc:'Every 5th shot fires a free echo.',apply(upg){if(upg.echoFire)return; upg.echoFire=true;}},
  {name:'Split Shot',tag:'OFFENSE',icon:'⋔',damageRole:'ceiling',desc:'Shots split on first wall bounce.',apply(upg){if(upg.splitShot||upg.bounceTier===0)return; upg.splitShot=true; raiseDamageFloor(upg);},evolvesWith:['Ricochet'],evolvedVersion:{name:'Fracture',icon:'⋔+',desc:'Bounce splits into 3 and gains damage.',apply(upg){if(upg.splitShot||upg.bounceTier===0)return; upg.splitShot=true; upg.splitShotEvolved=true; upg.denseDamageMult=(upg.denseDamageMult||1)*1.2; raiseDamageFloor(upg);}}},
  {name:'Volatile Rounds',tag:'OFFENSE',icon:'💢',damageRole:'ceiling',desc:'Pierce shots burst on the last hit.',apply(upg){if(upg.volatileRounds||upg.pierceTier===0)return; upg.volatileRounds=true; raiseDamageFloor(upg);},evolvesWith:['Pierce'],evolvedVersion:{name:'Chain Reaction',icon:'💢+',desc:'Every pierced hit bursts.',apply(upg){if(upg.volatileRounds||upg.pierceTier===0)return; upg.volatileRounds=true; upg.volatileAllTargets=true; raiseDamageFloor(upg);}}},
  ...
  {name:'Vampiric Return',tag:'SURVIVE',icon:'🩸',desc:'Kills heal you and grant charge. Room-capped.',apply(upg){if(upg.vampiric)return; upg.vampiric=true;}},
  ...
  {name:'Late Bloom',tag:'OFFENSE',icon:'🌱',damageRole:'ceiling',desc:'Damage scales by room. -6% speed.',apply(upg){if(hasLateBloomVariant(upg))return; upg.lateBloomVariant='power'; raiseDamageFloor(upg);}},
  {name:'Swift Bloom',tag:'UTILITY',icon:'🍃',damageRole:'neutral',desc:'Speed scales by room. +6% damage taken.',apply(upg){if(hasLateBloomVariant(upg))return; upg.lateBloomVariant='speed';}},
  {name:'Guard Bloom',tag:'SURVIVE',icon:'🛡️',damageRole:'defense',desc:'Defense scales by room. -6% damage.',apply(upg){if(hasLateBloomVariant(upg))return; upg.lateBloomVariant='defense';}},
  {name:'Escalation',tag:'OFFENSE',icon:'📈',damageRole:'ceiling',desc:'+3% damage per kill this room. Max +60%.',apply(upg){if(upg.escalation)return; upg.escalation=true; raiseDamageFloor(upg);}},
  {name:'Spread Shot',tag:'OFFENSE',icon:'⬄',damageRole:'neutral',desc:'3-shot cone. Costs 1 more charge, adds 35% spread damage, and +1 spread pierce.',apply(upg){if(upg.spreadShot)return; upg.spreadShot=true; upg.spreadShotDamageMult=1.35; upg.spreadShotPierceBonus=1; syncChargeCapacity(upg);}},
  ...
  {name:'Payload',tag:'OFFENSE',icon:'💣',desc:'Shots explode on impact. Base blast is larger.',requires:upg=>upg.biggerBulletsTier>0,apply(upg){if(upg.payload)return; upg.payload=true;}},
  ...
  {name:'Tether Orbit',tag:'UTILITY',icon:'🪢',desc:'Orbit spheres slow nearby danger bullets.',requires:upg=>upg.orbitSphereTier>0,apply(upg){if(upg.tetherOrbit)return; upg.tetherOrbit=true;}},
...
const LEGENDARY_SEQUENCES = [
  {
    ...
    boon: { name:'BULWARK', tag:'LEGENDARY', icon:'🏛️', desc:'Shield Burst hits 8-way. Mirror reflects deal ×2 damage.',
      apply(upg){ upg.aegisTitan=true; } }
  },
  {
    id: 'conduit',
    check: (h) => h.filter((n) => n === 'Orbit Spheres').length >= 2 &&
                  ['Tether Orbit','Orbital Focus','Charged Orbs','Absorb Orbs'].some((n) => h.includes(n)),
    boon: { name:'CONDUIT', tag:'LEGENDARY', icon:'⚡', desc:'Orbs link into a lightning web. More spheres strengthen the arcs.', requires: upg => upg.orbitSphereTier >= 2 && !upg.berserker && upg.titanTier === 0, apply(upg){ if(upg.conduit || upg.berserker || upg.titanTier > 0 || upg.orbitSphereTier < 2) return; upg.conduit = true; upg.conduitArcDmg = getConduitArcDamage(upg); upg.conduitArcTickMs = 120; } },
  },
  {
    id: 'ghostFlow',
    check: (h) => ['Kinetic Harvest','Quick Harvest'].every(n => h.includes(n)) &&
                   (h.includes('Slipstream') || h.includes('Flux State')),
    boon: { name:'GHOST FLOW', tag:'LEGENDARY', icon:'🌊', desc:'Absorb value scales with speed. Near-miss charge doubles.',
      apply(upg){ upg.ghostFlow=true; } }
  },
  {
    id: 'corona',
    check: (h) => h.filter(n => n==='Ring Blast').length >= 3,
    boon: { name:'CORONA', tag:'LEGENDARY', icon:'☀️', desc:'Ring shots pierce once. Every 3rd ring kill bursts.',
  ...
  },
  {
    ...
    boon: { name:'FINAL FORM', tag:'LEGENDARY', icon:'💀', desc:"Dead Man triggers at ≤15% HP. Low-HP kills grant charge.",
      apply(upg){ upg.finalForm=true; } }
  },
  {
    id: 'colossus',
    check: (h) => h.filter(n => n==='Titan Heart').length >= 3,
    boon: { name:'COLOSSUS', tag:'LEGENDARY', icon:'⬡', desc:'Damage releases a max-HP shockwave that greys bullets and halves Titan slow.',
      apply(upg){ upg.colossus=true; } }
  },
  {
    id: 'bloodMoon',
    check: (h) => ['Vampiric Return','Crimson Harvest','Sanguine Burst'].every(n => h.includes(n)),
    boon: { name:'BLOOD MOON', tag:'LEGENDARY', icon:'🩸', desc:'Kills restore 8 HP, add charge stacks, and drop 3 grey bullets.',
      apply(upg){ upg.bloodMoon=true; } }
  },
  {
    id: 'voidWalker',
    check: (h) => ['Phase Dash','Slipstream','Gravity Well'].every(n => h.includes(n)),
    boon: { name:'VOID WALKER', tag:'LEGENDARY', icon:'🌊', desc:'Dashing leaves a 2s void zone that slows danger bullets.',
      apply(upg){ upg.voidWalker=true; } }
  },
  {
    id: 'phantomRebound',
    check: (h) => ['Pierce','Ricochet','Long Reach'].every(n => h.includes(n)),
    boon: { name:'PHANTOM REBOUND', tag:'LEGENDARY', icon:'👻', desc:'Last wall bounce becomes a grey charge bullet. Long Reach doubles.',
