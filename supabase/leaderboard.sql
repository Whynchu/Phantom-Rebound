...
create index if not exists leaderboard_scores_score_idx
  on public.leaderboard_scores (score desc, created_at desc);
...
create index if not exists leaderboard_scores_name_idx
  on public.leaderboard_scores (player_name, created_at desc);
...
begin
  ...
  if p_game_version is null or char_length(trim(p_game_version)) < 1 or char_length(trim(p_game_version)) > 24 then
    raise exception 'invalid game_version';
  end if;

  insert into public.leaderboard_scores (player_name, score, room, game_version)
  values (v_name, p_score, p_room, trim(p_game_version));
...
as $$
  with filtered as (
    ...
    where
      (coalesce(p_period, 'daily') <> 'daily' or ls.created_at >= date_trunc('day', now()))
      and (
